...
//		// 定时事件触发：【waitRinging状态】下n秒后触发【waitRinging超时事件】
		...
		builder.defineTimedState(State.waitReport, 50, 0, Event.timeoutForReport,
				null);
		builder.defineTimedState(State.waitReport, 50, config.getStatemachineSessiontestInterval(), Event.timeToTestSession,
				null);
		//定时事件触发：【Connect状态】下n秒后触发【放音收号事件】
		builder.defineTimedState(State.waitReport, 10, 0, Event.PA,
				null);
...
// 状态跃迁：【初始状态】下触发【准备开始连接事件】时，并调用方法(发起连接)
	...
	//	builder.externalTransition().from(State.idle).to(State.waitAlerting).on(Event.intoFail);
		...
		builder.externalTransition().from(State.waitRinging).to(State.waitReport).on(Event.receiveConnnecting).callMethod("receiveConnnecting");

		// 状态跃迁：【waitConnect】下触发【receiveConnnecting】时，跳到【waitReport】
		builder.externalTransition().from(State.waitConnect).to(State.waitReport).on(Event.receiveConnnecting).callMethod("receiveConnnecting");
		...
		// 状态跃迁：【waitReport】下触发【PA】时，调用方法(PLAYONLY)
		builder.internalTransition().within(State.waitReport).on(Event.PA).callMethod("playOnly");
		// 状态跃迁：【waitReport】下触发【playEnd】时，调用方法(playEnd)
		builder.internalTransition().within(State.waitReport).on(Event.playEnd).callMethod("playEnd");

		// 状态跃迁：【waitReport】下触发【askForReplayKey】时，跳到【waitReport】，并调用方法(askForReplayKey)
		builder.internalTransition().within(State.waitReport).on(Event.askForReplayKey).callMethod("askForReplayKey");
		// 状态跃迁：【waitReport】下触发【testSessionFail】时，跳到【Idel】，并调用方法(releaseSelf)
		builder.externalTransition().from(State.waitReport).to(State.released).on(Event.testSessionFail).callMethod("testSessionFail");
		// 【waitReport】下触发【timeToTestSession】时，调用方法(testSession)
		builder.internalTransition().within(State.waitReport).on(Event.timeToTestSession).callMethod("testSession");
		// 状态跃迁：【waitReport】下触发【receiveRelease】时，跳到【Idel】，并调用方法(releaseSelf)
		builder.externalTransition().from(State.waitReport).to(State.released).on(Event.receiveRelease).callMethod("receiveRelease");
